    ...
    */
    ...
    private static func generateStoreURL(requestedURL: NSURL?, modelName: String) -> NSURL {
            ...
            do {
                ...
                return requestedURL ?? NSURL(fileURLWithPath: "\(modelName).sqlite", relativeToURL: applicationDirectory)
            }
            catch {
                fatalError("Could not locate Store URL: \(requestedURL) error: \(error)")
            }
        #endif
    }
